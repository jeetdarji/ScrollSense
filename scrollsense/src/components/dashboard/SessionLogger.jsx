...
export default function SessionLogger({ prefilledData, onCompletePrefill }) {
  ...
  const todayTotal = todaySummary.totalMinutes + todayAutoMinutes;
          ...
          {state === 'idle' && (
            ...
            >
              ...
              <div className="mt-2">
                ...
                {(todaySessions.length > 0 || todayAutoMinutes > 0) ? (
                  <>
                    <div className="w-full h-[6px] flex gap-1 mb-2">
                      {todayAutoMinutes > 0 && (
                        <div 
                          className="h-full bg-red-600"
                          style={{ flexGrow: todayAutoMinutes }}
                          title={`YouTube Auto: ${todayAutoMinutes}m`}
                        />
                      )}
                    ...
                    <div className="text-xs uppercase tracking-widest text-[#A1A1AA] font-medium flex justify-between">
                      <span>TOTAL TODAY: {todayTotal} MIN</span>
                      <div className="text-[10px] flex gap-2">
                        {todayAutoMinutes > 0 && <span className="text-red-500">YT: {Math.round(todayAutoMinutes)}M</span>}
                        {todaySummary.totalMinutes > 0 && <span>MANUAL: {Math.round(todaySummary.totalMinutes)}M</span>}
                      </div>
                    </div>
                  </>
                ) : (
