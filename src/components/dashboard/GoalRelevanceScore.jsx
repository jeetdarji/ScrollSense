...
export default function GoalRelevanceScore({ className }) {
  const { isConnected, isLoading, currentWeek, goalScoreHistory, contentSuggestions } = useYouTubeData();
  ...
  if (!isConnected) {
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} viewport={{ once: true }} className={clsx("border-2 border-[#3F3F46] bg-[#09090B] p-6 md:p-8", className)}>
        <YouTubeConnectPrompt featureName="GOAL RELEVANCE" featureId="F13" />
      </motion.div>
    );
  }
  ...
  const diff = currentWeek.changeVsLastWeek.goal;
  const diffFromAvg = currentWeek.goal - 8;
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} viewport={{ once: true }} className={clsx("border-2 border-[#3F3F46] bg-[#09090B] p-6 md:p-8 flex flex-col h-full", className)}>
        ...
        {!hasHistory && (
          <div className="flex items-center gap-2 mt-3">
            ...
            <span className="text-xs text-[#3F3F46] uppercase tracking-wider">
              Trend line builds over 2 weeks — {Math.max(0, 2 - currentWeek.weeksOfData)} more week(s) to go.
            </span>
          </div>
        )}
      </div>
      ...
      <div className="mt-2 mb-6">
        ...
        <div className="flex flex-col gap-3">
          {[
            { label: 'GOAL RELEVANT', value: currentWeek.goal, color: '#DFE104', textColor: '#DFE104' },
            { label: 'GENUINE INTEREST', value: currentWeek.interest, color: 'rgba(250,250,250,0.4)', textColor: '#A1A1AA' },
            { label: 'RANDOM / JUNK', value: currentWeek.junk, color: '#3F3F46', textColor: '#3F3F46' }
          ].map((item, i) => (
